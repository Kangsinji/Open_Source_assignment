...
class Tetris(object):
    ...
    def set_background_image(self, image_path):
        try:
            ...
            # 원본 배경 이미지의 크기를 가져옵니다
            bg_width, bg_height = self.background_image.get_size()

            # 게임 화면의 크기에 맞게 배경 이미지를 조정합니다
            self.background_image = pygame.transform.scale(self.background_image, (self.resx, self.resy))
            self.screen = pygame.display.set_mode((self.resx, self.resy))
            # 배경 이미지를 게임 화면에 표시합니다
    ...
    def run(self):
        ...
        # Select and set the background image
        '''self.background_image_path = self.select_background_image()'''
        if self.background_image_path:
            self.set_background_image(self.background_image_path)
            background_image = pygame.image.load(self.background_image_path)
            self.resx = background_image.get_width()
            self.resy = background_image.get_height()
            self.screen = pygame.display.set_mode((self.resx, self.resy))

        # Initialize mixer before loading music
        pygame.mixer.init()
        # Load music file
        pygame.mixer.music.load("Tetris-Bradinsky.mp3")
        # Start playing music (-1 is for infinite loop)
    ...
    def detect_line(self):
        ...
        if self.active_block is not None and self.active_block.shape is not None:
    ...
    def get_block(self):
        ...
        if self.new_block:
            ...
            new_block = block.Block(data[0], self.start_x, self.start_y, self.screen, data[1], data[2])
            
            self.blk_list.append(new_block)
            self.active_block = new_block  # Assign the newly created block to active_block
            self.new_block = False
    ...
    def draw_game(self):
        ...
        # all tetris blocks
        window_width, window_height = 36,36
        pygame.display.set_caption("Pygame Background Image Selector")
        if self.background_image_path:
                self.screen.blit(self.background_image, (0, 0))
        self.draw_board()
        for blk in self.blk_list:
...
if __name__ == "__main__":
    tetris_game = Tetris(16, 30)
    background_image_path = tetris_game.select_background_image()  # 이미지 선택

    if background_image_path:
        tetris_game.set_background_image(background_image_path)  # 이미지 설정

        # 이미지를 선택했을 때만 게임 실행
        print("이미지 선택 완료. 게임을 시작합니다.")
        tetris_game.run()
    else:
        print("배경 이미지 선택을 취소하였습니다. 게임을 종료합니다.")
        pygame.quit()
        sys.exit()
